        ...
        for r in self.rules:
            for pat in r.get("block_patterns", []):
                if re.search(pat, user_text, flags=re.IGNORECASE):
                    ...
                    break
        if matched:
            return PolicyResult(action=self.overrides.get("blocked", "REFUSE"), matched_rules=matched)
        return PolicyResult(action=self.overrides.get("safe", "ALLOW"), matched_rules=[])